...
local _coordinates_it = function(state, pos)
  local array, dim_count, d = state[1], state[2], state[3]
  local ndim = array.ndim

  if dim_count == 0 then
    dim_count = array.shape[ndim-1]
    pos[d] = -1
    d = d - 1
    ...
    pos[d] = pos[d] + 1
    while pos[d] >= array.shape[d] do
      if pos[0] == array.shape[0] then
        return nil
      end
      pos[d] = 0
      d = d - 1
...
-- array iterator that yields a coordinate which is a zero-based table
  ...
  -- -- trying to improve performance with stateless iterators
  -- -- vs. closures does not work, even though they can be 
  -- -- compiled by LUAJIT:
  -- local state = {}
  -- state[1] = self
  -- state[2] = dim_count
  -- state[3] = d
  -- return _coordinates_it, state, pos
  if self.ndim == 1 then
    return function()
      if dim_count == 0 then
        return nil
